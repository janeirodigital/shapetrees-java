...
public class HttpResourceAccessor implements ResourceAccessor {
    ...
    protected ManageableInstance.Resource generateResource(URL url, DocumentResponse response) throws ShapeTreeException {
        ...
        if (Boolean.TRUE.equals(isManager)) {
            ...
            final URL managedResourceUrl;
            try {
                managedResourceUrl = new URL(url, url.getPath().replaceAll("\\.shapetree$", ""));
            } catch (MalformedURLException e) {
                throw new ShapeTreeException(500, "can't calculate managed resource for shape tree manager <" + url + ">");
            }
    ...
    static Boolean calculateIsManager(URL url, boolean exists, ResourceAttributes parsedLinkHeaders) {
        ...
        // with a manager target, it is not a manager resource (because it is pointing to one)
        ...
        // TODO - Look for the presence of a rel=manages header instead
